/*
  HLSAssetPersistenceManager.swift
  Project-Template

  Created by Miguel Paysan on 7/5/21.

    HLSAssetPersistenceManager is the main class used to manage downloading HLS streams.
    Includes API for downloading, cancelling, deleting existing assets from user's device, and monitoring download progress
 */

import Foundation
import AVFoundation

class HLSAssetPersistenceManager: NSObject {
    // MARK: Properties
    
    /// Singleton for HLSAssetPersistenceManager
    static let sharedManager = HLSAssetPersistenceManager()
    ...
    override private init() {
        ...
        let backgroundConfiguration = URLSessionConfiguration.background(withIdentifier: K.URL_CONFIG_BACKGROUND_ID)
...
//                self.activeDownloadsMap.removeValue(forKey: assetDownloadTask)
                //TODO - Do we need to pass the whole Video Attribute parameters? Or good enough for just the name. Maybe encode (Name + ID) , or just IDfor later
                let video = Video.with(name: assetName)
    ...
    /// - Tag: DownloadVideo
    func downloadVideo(for asset: Asset) {
        ...
        task.taskDescription = asset.video.name
        ...
        userInfo[Asset.Keys.name] = asset.video.name
    ...
    func assetForStream(withName name: String) -> Asset? {
        ...
        for (_, assetValue) in activeDownloadsMap where name == assetValue.video.name {
            asset = assetValue
            break
        }
    ...
    func localAssetForVideo(withName name: String) -> Asset? {
...
//            let stream = StreamListManager.shared.stream(withName: name)
            //TODO - Do we need to pass the whole Video Attribute parameters? Or good enough for just the name. Maybe encode (Name + ID) , or just IDfor later
            let video = Video.with(name: name)
            
            asset = Asset(video: video, urlAsset: urlAsset)
    ...
    func downloadState(for asset: Asset) -> Asset.DownloadState {
        ...
        if let localFileLocation = localAssetForVideo(withName: asset.video.name)?.urlAsset.url {
        ...
        for (_, assetValue) in activeDownloadsMap where asset.video.name == assetValue.video.name {
            return .downloading
        }
    ...
    func deleteAsset(_ asset: Asset) {
        ...
        do {
            if let localFileLocation = localAssetForVideo(withName: asset.video.name)?.urlAsset.url {
                ...
                userDefaults.removeObject(forKey: asset.video.name)
 ...
 */
extension HLSAssetPersistenceManager: AVAssetDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if let error = error as NSError? {
            ...
            case (NSURLErrorDomain, NSURLErrorCancelled):
                 ...
                 */
                guard let localFileLocation = localAssetForVideo(withName: asset.video.name)?.urlAsset.url else { return }
                ...
                do {
                    ...
                    print("An error occured trying to delete the contents on disk for \(asset.video.name): \(error)")
                }

                userInfo[Asset.Keys.downloadState] = Asset.DownloadState.notDownloaded.rawValue

            case (NSURLErrorDomain, NSURLErrorUnknown):
                fatalError("Downloading HLS streams is not supported in the simulator. errorCode:\(error.code)")
            ...
            case (NSURLErrorDomain, NSURLErrorBackgroundSessionInUseByAnotherProcess):
                fatalError("NSURLErrorBackgroundSessionInUseByAnotherProcess")
            case (AVFoundationErrorDomain, -11800):
                fatalError("@@ AVFoundationError Domain:\(-11800)  ")
            default:
                fatalError("An unexpected error occured \(error.domain) | errorCODE:\(error.code)")
            }
        } else {
            do {
                let bookmark = try downloadURL.bookmarkData()

                userDefaults.set(bookmark, forKey: asset.video.name)
            } catch {
    ...
    func urlSession(_ session: URLSession, aggregateAssetDownloadTask: AVAggregateAssetDownloadTask,
        ...
        aggregateAssetDownloadTask.taskDescription = asset.video.name
